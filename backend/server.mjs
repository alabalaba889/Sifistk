...
const ORIGIN=(process.env.SIFISTK_PUBLIC_ORIGIN||`http://${ORIGIN_HOST}:${PORT}`).replace(/\/$/,"");
...
async function route(req,res){
  ...
  if(req.method==="GET"&&p==="/api/config")return json(res,200,{version:VERSION,inviteRequired:INVITE_REQUIRED,portal:"/app/",agent:{enabled:!!OPENAI_API_KEY,model:AI_MODEL,webSearch:AI_WEB_SEARCH,reasoningEffort:AI_REASONING_EFFORT},extensionDownloadUrl:EXTENSION_DOWNLOAD_URL});
  ...
  if(req.method==="GET"&&p==="/api/extension/version")return json(res,200,{latestVersion:VERSION,minVersion:"9.0.1",downloadUrl:EXTENSION_DOWNLOAD_URL},extHeaders());
