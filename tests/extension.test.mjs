...
test("extension manifest is MV3 and permissions match implemented APIs",async()=>{
  ...
  assert.deepEqual(m.host_permissions.sort(),["http://127.0.0.1:8787/*","http://localhost:8787/*"]);
...
test("all extension JavaScript parses",async()=>{
  ...
  assert.match(core,/Não foi possível conectar ao servidor da Sifistk/);
  assert.match(agent,/chrome\.permissions\.request/);
  assert.match(agent,/Não foi possível conectar ao servidor da Sifistk/);
});
...
test("safe agent controls exist",async()=>{
  ...
  assert.match(agent,/ensureServerPermission/);
});